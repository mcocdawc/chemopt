...
CO_INIT = path.join(CURRENT_DIRECTORY, 'src', 'chemopt', '__init__.py')
CO_TUTORIAL = path.join(CURRENT_DIRECTORY, 'docs', 'source', 'tutorial.rst')
...
#     sed_inplace(CO_INIT, '_git_hash = .*', '_git_hash = "{}"'.format(git_hash))
#
#
# def get_git_hash():
#     """Get git hash
...
def update_git_branch():
    """Update co.__init__.py"""
    branch = get_git_branch()
    sed_inplace(CO_INIT, '_git_branch = .*',
...
def update_doc_tutorial_url():
    ...
    sed_inplace(CO_TUTORIAL, pattern, replacement)
